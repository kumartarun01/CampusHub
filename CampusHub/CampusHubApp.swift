...
// ─────────────────────────────────────────────
// MARK: - App State (single source for auth)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// MARK: - RootView  (switch on AppState)
// ─────────────────────────────────────────────
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                ZStack {
                    RoundedRectangle(cornerRadius: 20).fill(Color(white: 0.18)).frame(width: 90, height: 90)
                    RoundedRectangle(cornerRadius: 10).stroke(Color.white, lineWidth: 3).frame(width: 44, height: 44)
                }
                .padding(.bottom, 28)
            ...
            ZStack {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 0) {
                        ...
                        VStack(spacing: 14) {
                            ZStack {
                                RoundedRectangle(cornerRadius: 20).fill(Color(white: 0.18)).frame(width: 72, height: 72)
                                RoundedRectangle(cornerRadius: 8).stroke(Color.white, lineWidth: 2.5).frame(width: 36, height: 36)
                            }
                            Text("CampusHub").font(.system(size: 28, weight: .bold)).foregroundColor(.white)
                            Text("Welcome back 👋").font(.system(size: 15)).foregroundColor(Color(white: 0.5))
                        }
                        .padding(.top, 60).padding(.bottom, 40)

                        // Form
                        VStack(spacing: 16) {
                            loginField("Email", icon: "envelope", text: $email, secure: false, show: .constant(false))
                            loginField("Password", icon: "lock", text: $password, secure: !showPassword, show: $showPassword, showToggle: true)

                            HStack {
                                Spacer()
                                NavigationLink(destination: ForgotPasswordScreen()) {
                                    Text("Forgot Password?")
                                        .font(.system(size: 13, weight: .medium)).foregroundColor(Color(white: 0.55))
                                }
                            }.padding(.top, 2)
                            ...
                            Button {} label: {
                                HStack(spacing: 10) {
                                    ...
                                    Text("Continue with Google").font(.system(size: 15, weight: .medium)).foregroundColor(.white)
                                }
                                .frame(maxWidth: .infinity).frame(height: 52)
                        ...
                        HStack(spacing: 4) {
                            Text("Don't have an account?").font(.system(size: 14)).foregroundColor(Color(white: 0.45))
                            NavigationLink(destination: SignUpScreen()) {
                                Text("Sign Up").font(.system(size: 14, weight: .semibold)).foregroundColor(.white)
                            }
                        }
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 12) {
                ...
                Group {
                    ...
                    else { TextField("", text: text).autocapitalization(.none) }
                }
...
struct OnboardingScreen: View {
    ...
    private let steps: [(title: String, body: String, icon: String)] = [
        ("Discover events\nhappening around\ncampus", "Never miss a workshop, fest, or club activity. Everything in one feed.", "calendar.badge.clock"),
        ("Join clubs &\nstay connected",               "Follow your favourite clubs and get instant updates on meetings and events.", "person.3.fill"),
        ("RSVP with\none tap",                         "Save events, RSVP instantly and get reminders so you never miss out.", "bookmark.fill")
    ]
