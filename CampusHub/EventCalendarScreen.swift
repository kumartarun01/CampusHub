...
struct EventCalendarScreen: View {
    ...
    private let startOffset = 2    // April 2025 starts on Tuesday

    private struct CalEvent: Identifiable {
        let id = UUID(); let title, time, tag: String
    }

    private let events: [CalEvent] = [
        CalEvent(title: "Tech Talk: AI in 2025", time: "Apr 3 · 9AM · Hall A",     tag: "Free"),
        CalEvent(title: "Coding Marathon",        time: "Apr 7 · 10AM · Lab 3",     tag: "Free"),
        CalEvent(title: "Art Exhibition",         time: "Apr 5 · 10AM · Gallery",   tag: "Cultural")
    ]
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 12) {
                    ...
                    HStack {
                        Button {} label: {
                            Image(systemName: "chevron.left")
                                .foregroundColor(Color(white: 0.5)).font(.system(size: 14))
                        }
                        Spacer()
                        Text("April 2025")
                    ...
                    // Day grid (28 days + offset blanks)
                    let cols = Array(repeating: GridItem(.flexible(), spacing: 0), count: 7)
                    LazyVGrid(columns: cols, spacing: 4) {
                        // blank cells for offset
                        ForEach(0..<startOffset, id: \.self) { _ in Color.clear.frame(height: 36) }
...
#Preview {
    struct Wrap: View {
        ...
        var body: some View { EventCalendarScreen(selectedTab: $tab) }
    }
    return Wrap()
}
